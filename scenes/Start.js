...
class Start extends Phaser.Scene {
  ...
  create() {
    ...
    // Title
    game.gameTitle = this.add.text(450, 100, "Bc Alien", {
    ...
    // Options
    game.startOptions = this.add.text(630, 321.5, "Iniciar", {
      fontFamily: "kenneyPixel",
      fontSize: 60,
      color: "#4D4D4D"
    });
    game.instructionsOption = this.add.text(630, 381.5, "Como jogar", {
      fontFamily: "kenneyPixel",
      fontSize: 60,
      color: "#4D4D4D"
    });
    ...
    game.selectInstruction = this.add.text(510, 580, "Pressione para a direita para selecionar", {
      fontFamily: "upheaval",
      fontSize: 30,
      color: "#4D4D4D"
    });
  ...
  update() {
    ...
    if (game.chooseCursor.selection === "github") {
      ...
      if (Phaser.Input.Keyboard.JustDown(RIGHT)) {
        open("https://github.com/BrunoCosta19");
      }
    } else {
